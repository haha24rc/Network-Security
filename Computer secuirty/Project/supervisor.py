...
def recv_thread(key,s,q): #receive msgs awaiting approval
    ...
    while(size != b''):
        ...
        name, cost, amount, sig1, PUP, timestamp = msg.split(b'|,|') #recover msg elements
...
def main():
    ...
    PRA, PUA, ks = authentificate(s,"supervisor") #authenticate the supervisor
    ...
    while(True):
        try: #display msgs as they are added to the queue, prompt the supervisor for approval
            if (q.qsize() != 0):
                rmsg = q.get()
                ...
                if (userinput.lower() == "yes"): #send signed msg is approved
                    hash = SHA3_256.new(name + b',' + cost + b',' + amount)
    ...
    s.shutdown(socket.SHUT_RDWR) #close connection to the mail server
    s.close()
